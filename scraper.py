...
def getvalbyjsonselector(selectorial, dictorial):
    try:
        ...
        if len(selectorial) == 1:
            ...
            #print('FOUR')
            value = dictorial[selectorial[0]]
            selectorial.pop(0)
            selectorial = '>>>'.join(selectorial)
            return getvalbyjsonselector(selectorial, value)
    except:
        print(traceback.format_exc())
...
for scrapsite in jsonscrapsites:
    ...
    # >>> GET THE JSON/API <<< #
    if scrapsite['scrapefield']['scrapetype'] == 'jsonapi_morph_io':
        jsonmain = ''
        try:
            r = requests.get(scrapsite['scrapeurl'])
            jsonmain = r.json()
                            ...
                            if domainmisc_array[i]:
                                ...
                                if domainmisc_array[(i-1)] == 'pa_category':
                                    if len(domainmisc_array[i]) > 0:
                                        ...
                                        count = 0
                                        for el in domainmisc_array[i]:  
                                            prodcat_array.append(domainmisc_array[i][count])
                                            count = count + 1
                                        if prod_categories != '':
                                ...
                                if domainmisc_array[(i-1)] == 'pa_size':
                                    ...
                                    if len(domainmisc_array[i]) > 0:
                                        ...
                                        for el in domainmisc_array[i]:
                                            ...
                                            prod_sizes.append(domainmisc_array[i][count])
                                            count = count + 1
                                ...
                                if type(domainmisc_array[i]) is list:
                                    count = 0
                                    for el in domainmisc_array[i]:
                                        domainmisc_array[i][count] = str(el)
                                        count = count + 1
                                else:
